...
extension Bind {
    ...
    public var value: PostgresDataType {
 ...
 */
extension Bind {
    fileprivate static func parse(type: FieldType.Supported, configuration: Configuration, value: UnsafeMutablePointer<Int8>, length: Int) -> PostgresDataType {
        switch type {
        case .bool:
        ...
        case .date:
            let date = BinaryUtils.parseDate(value: value, isInteger: configuration.hasIntegerDatetimes)
            return .date(date)
    ...
    fileprivate static func parse(type: FieldType.ArraySupported, configuration: Configuration, value: UnsafeMutablePointer<Int8>, length: Int) -> PostgresDataType {
    ...
    private static func parse(type: FieldType.ArraySupported, configuration: Configuration, dimensionLengths: [Int], pointer: inout UnsafeMutablePointer<Int8>) -> PostgresDataType {
        // Get the length of the array
        let arrayLength = dimensionLengths[0]
        
        // Create elements array
        var values: [PostgresDataType] = []
